...
function ModuleHandler ($src, $config) {
  this.src = $src;
  this.config = $config;
  this.name = this.src;
...
function triggerTask(_argument) {
  if (!_$4.isUndefined(_argument.compile)) {
    processUserFiles();
  }

  if (!_$4.isUndefined(_argument.watch)) ;
}
...
function sofy() {
  ...
  program.option('-c, --config [location]', 'Custom Location For Config File', './sofy.js').option('--compile', 'Triggers Build For All Files Listed In Config File').option('--create [location]', 'Location On Where To Create The Config File').parse(process.argv);

  if (!_$5.isUndefined(program.create)) {
